...
#else
int rr_interval __read_mostly = 24;
#endif
			  ...
			  int wake_flags)
{
	...
	/* This barrier is undocumented, probably for p->state? ?ã */
	smp_wmb();
	...
	/* state is a volatile long, ?©ã†?—ã¦?åˆ†?‹ã‚‰?ªã„ */
...
{
	/* NOT a real fix but will make voluntary preempt work. é¦¬é¹¿?ªäº‹ */
